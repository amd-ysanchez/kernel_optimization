...
Install **rocisa** locally since it depends with Tensile:
...
For this we run the script *configure.py*

With the following arguments:

```
usage: configure.py [-h] [--device DEVICE] [--thr THR] [--architecture ARCHITECTURE] [--workdir WORKDIR] hipblaslt_path gemm_log
...
```
python configure.py /path/to/rocm-libraries/projects/hipblaslt/ /path/to/hipblaslt.log -a gfx950 --workdir WORKDIR
```
...
options:
  ...
  --log_summary LOG_SUMMARY
                        CSV file generated by configure.py script. Will be used to report the weighted uplift.
